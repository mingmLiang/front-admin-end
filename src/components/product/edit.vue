...
<!--       <el-form-item label="是否上架">
      ...
      <el-form-item label="商品规格" prop="spe_name">
        ...
        <el-row :gutter="24">
          ...
          <el-col :span="12"><div class="grid-content bg-purple"><el-button  type="primary" @click="spe_add()">添加</el-button>
          <!-- <el-button  type="primary" @click="form.spe_name=[]">清空</el-button> -->
          </div></el-col>
        </el-row>
      </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      submit: function () {
        ...
        let this_ = this;
        http.ajaxPost(this, api.pro_edit, {formdata: dataForm}, function (data) {
          if (data.code === 200) {
            this_.$router.push({path: '/product/list'});
          } else {}
        });
      },
      // 重置输入表单
    ...
    created () {
      ...
      let obj = JSON.parse(sessionStorage.getItem('edit_pro_qtt'));
      this.form.pro_name = obj.pro_name;
      this.form.image = obj.image.split(';');
      this.form.price = obj.price;
      this.form.sale_price = obj.sale_price;
      this.form.city = obj.city;
      this.form.decript = obj.decript;
      this.form.sor_id = obj.sor_id;
      this.form.pro_id = obj.pro_id;
      let tags = [];
      obj.tags.forEach(function (e, i) {
        tags.push(e.tags_id);
      });
      this.form.tags = tags;
      let spes = [];
      obj.spes.forEach(function (e, i) {
        spes.push(e.spe_name);
      });
      this.form.spe_name = spes;
      console.log(JSON.parse(sessionStorage.getItem('edit_pro_qtt')));
    }
  };
</script>
